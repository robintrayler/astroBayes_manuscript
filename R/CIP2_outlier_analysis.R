...
age_model <- function(i, 
  ...
  # make one random data an outlier -----------------------
  o <- sample(1:size, 1)
  geochron_data$age[o] <- geochron_data$age[o] + 
    geochron_data$age_sd[o] * sample(-4:4, 1)
  # run the model -----------------------------------------
  ...
  model_folder <- paste0('./results/outlier_analysis/CIP2_', 
                         size, 
                         '_ages/')
