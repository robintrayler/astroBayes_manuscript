...
![Synthetic testing data sets used for model validation. A, D) The synthetic cyclostratigraphic records for TD1 and CIP2. B, E) True age-depth models for both data sets. The colored probability distributions are the synthetic radioisotopic dates used for model stability testing (@tbl:testing_dates). C, F) Evolutive harmonic analysis of panels A (3 m window size, 0.1 m step size) and D (2 m window size, 0.1 m step size). Lighter colors indicate higher spectral amplitude. The horizontal dashed lines are layer boundary positions (*z*) chosen by visual inspection of the evolutive harmonic analysis results.](./figures/final figures/figure_2.pdf){#fig:testing_data width=100%}
